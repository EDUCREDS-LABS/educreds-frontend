...
const NetworkStat = ({ label, value, sub, icon: Icon, trend }: any) => (
  <div className="p-6 bg-neutral-900/50 backdrop-blur-xl border border-white/5 rounded-[24px] group hover:bg-neutral-800/50 transition-all">
    ...
    <p className="text-[9px] text-neutral-600 font-bold uppercase tracking-wider mt-1">{sub}</p>
  </div>
);

const InfraModule = ({ title, desc, icon: Icon, features, color }: any) => (
  <Card className="border-none shadow-2xl bg-neutral-900 overflow-hidden group">
    <div className={cn("h-1 w-full bg-gradient-to-r", color)} />
    <CardContent className="p-10 space-y-8">
      <div className="flex items-center gap-6">
        <div className={cn("size-16 rounded-2xl flex items-center justify-center shadow-xl group-hover:scale-110 transition-transform", color.replace('from-', 'bg-').split(' ')[0] + '/10')}>
          <Icon className={cn("size-8", color.replace('from-', 'text-').split(' ')[0])} />
        </div>
        <div>
          <h3 className="text-2xl font-black text-white tracking-tight">{title}</h3>
          <p className="text-[10px] font-black text-neutral-500 uppercase tracking-widest mt-1">Industrial Module v2.4</p>
        </div>
      </div>
      <p className="text-neutral-400 font-medium leading-relaxed">{desc}</p>
      ...
      <Button variant="outline" className="w-full h-12 rounded-xl border-white/5 hover:bg-white/5 text-white font-black text-[10px] uppercase tracking-widest">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center space-y-10">
          ...
          >
            ...
            <h1 className="text-6xl md:text-8xl font-black tracking-tighter leading-[0.9] mb-8">
              THE <span className="text-blue-500">BACKBONE</span> OF <br />
              ACADEMIC TRUST.
            </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 relative">
            ...
            {pipeline.map((step, i) => (
              <div key={i} className="relative z-10 space-y-6 group">
                <div className="size-20 rounded-[24px] bg-neutral-900 border border-white/5 flex items-center justify-center text-blue-500 shadow-2xl group-hover:bg-blue-500 group-hover:text-white transition-all mx-auto md:mx-0">
      ...
      <section className="py-32 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto relative overflow-hidden rounded-[48px] bg-blue-600 p-12 md:p-20 text-center space-y-10 group">
          ...
          <div className="relative z-10 space-y-6">
            <h2 className="text-5xl md:text-7xl font-black tracking-tighter text-white">READY TO DEPLOY?</h2>
            <p className="text-xl text-blue-100 max-w-2xl mx-auto font-medium">Join the infrastructure layer of global education. Start your institutional node deployment today.</p>
          </div>
          <div className="relative z-10 flex flex-col sm:flex-row gap-6 justify-center">
            <Link href="/register">
              <Button size="lg" className="h-16 px-12 rounded-2xl bg-white text-blue-600 hover:bg-neutral-100 font-black text-xs uppercase tracking-[0.2em] shadow-2xl">
